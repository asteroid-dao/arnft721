...
contract Minter is EIP712 {
  ...
  function mint (string memory short_id, bytes memory signature, string memory arweave_tx, uint nonce, bytes32 _extra, bytes memory signature2, uint _uint, uint _uint2) public {
    ...
    address to = ECDSA.recover(_hashTypedDataV4(keccak256(abi.encode(keccak256("NFT(bytes signature,string arweave_tx,uint256 nonce,bytes32 extra)"), long_id, keccak256(bytes(arweave_tx)), nonce, _extra))), signature2);
